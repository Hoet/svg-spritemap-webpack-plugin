...
module.exports = class SVGSpritemapPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tap(plugin, (compilation) => {
            ...
            // Extra checks when using variables
            if ( findVariables(spritemap).length ) {
                // Make sure this language supports variables
                if ( !['scss', 'sass'].includes(extension) ) {
                    compilation.warnings.push(new VariablesNotSupportedInLanguageWarning(extension));
                }

                // TODO: Throw when multiple variables with the same name but different default values are detected
            }
            ...
            compilation.hooks.optimizeAssets.tapAsync(plugin, (assets, callback) => {
                ...
                const stripped = stripVariables(compilation.assets[this.options.filename].source());
                compilation.assets[this.options.filename] = new RawSource(stripped);
