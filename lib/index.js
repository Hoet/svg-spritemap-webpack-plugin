...
const parseVars = require('./variable-parser');
    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tap(plugin, (compilation) => {
            ...
            compilation.hooks.optimizeAssets.tapAsync(plugin, (assets, callback) => {
                const parsed = parseVars(compilation.assets[this.options.filename].source());
                compilation.assets[this.options.filename] = new RawSource(parsed);
